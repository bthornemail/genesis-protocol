...
* 13. Transform Layer — Spatial Projection (Layer 3+)

This implements the canonical GENESIS transform from atoms to 3D coordinates.

See: =docs/RFC-GENESIS-TRANSFORM-0001.md= for formal specification.

#+BEGIN_SRC scheme :tangle genesis/layer.3/genesis-transform.scm
;; genesis-transform.scm (Guile)
;; Canonical transform: atom → (position, scale, rotation)
;; Implements RFC-GENESIS-TRANSFORM-0001

(use-modules (ice-9 math))
...
;; Class offset: private=0.0, protected=0.25, public=0.5
(define (class-offset cls)
  (cond ((equal? cls "public") 0.5)
        ((equal? cls "protected") 0.25)
        (else 0.0)))

;; Main transform: atom + options → position/scale/rotation
...
;; Convert transform to packed Float32 buffer layout: [x y z s rotY]
(define (to-buffer t)
...
(define (geometry-for-role role)
  (cond ((equal? role "kernel") "octahedron")
        ((equal? role "org") "icosahedron")
        ((equal? role "asset") "sphere")
        (else "box")))
#+END_SRC